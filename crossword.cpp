...
/*
 *Returns if a valid word is detected at location, clears Out paramter if valid word is found puts it in out.
 *
 */
bool crossword::valid_word_across(std::string &out, const int &row, const int &col) const {
	out.clear();
	if ((col - 1) < 0 ) {
		if ( cross[row][col] != '.' ) {
...
bool crossword::valid_word_down(std::string &out, const int &row, const int &col) const {
	out.clear();
	if ((row - 1) < 0 ) {
		if ( cross[row][col] != '.' ) {
